...
function submit_job {
    ...
    p_info "bsub $IRAP_LSF_PARAMS -q $QUEUE -n $THREADS -M $MAX_MEM -R 'span[hosts=1]'  -w '$WAIT_FOR_ID'  -o '`get_path2logfile`/$jobname-%J.out' -e '`get_path2logfile`/$jobname-%J.err' -J $jobname $cmd2e max_threads=$THREADS data_dir=$DATA_DIR max_mem=$MEM\""

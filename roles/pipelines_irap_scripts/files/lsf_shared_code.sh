...
function submit_job_status {
    ...
    $ECHO bsub $IRAP_LSF_PARAMS -M 1000 -R "rusage[mem=1000]" -q $QUEUE  -J "${jobname}n" $WAITFOR  irap_lsf_job_status.sh $jobname $JOB_ID  `get_maxmem $MEM` $LOG_DIR  $IRAP_PAR_CMD
    #2> /dev/null
    if [ $? != 0 ]; then
...
function submit_job {
    ...
    p_info "LSB_DOCKER_IMAGE=nunofonseca/irap_ubuntu:v0.7.0p1 LSB_DOCKER_OPTIONS=\"-v $DATA_DIR:$DATA_DIR\" bsub $IRAP_LSF_PARAMS -q $QUEUE -n $THREADS -M $MAX_MEM -R 'span[hosts=1]'  -w '$WAIT_FOR_ID'  -o '`get_path2logfile`/$jobname-%J.out' -e '`get_path2logfile`/$jobname-%J.err' -J $jobname $cmd2e max_threads=$THREADS data_dir=$DATA_DIR max_mem=$MEM\""
